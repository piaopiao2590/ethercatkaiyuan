...
foeApp::foeApp(QObject *parent) : QObject(parent)
{

}


void foeApp::ecx_FOEwrite_gxf(ecx_contextt *context, uint16 slave, char *filename, uint32 password, int psize, void *p, int timeout)
{
   ec_FOEt *FOEp, *aFOEp;
   int wkc;
...
void foeApp::ecx_FOEread_gxf(ecx_contextt *context, uint16 slave, char *filename, uint32 password, void *p, int timeout)
{
   ec_FOEt *FOEp, *aFOEp;
   int wkc;
   int32 dataread = 0;
...
//                        foeWriteHook(slave, packetnumber, psize);
...
/**
 * @brief foeApp::taskWrite
 * FOE写任务
 */
void foeApp::taskWrite(void)
{
    ecx_FOEwrite_gxf(&ecx_context, 1, (char *)&filename, password, filesize, filebuf, 4000000);
}
/**
 * @brief foeApp::taskRead
 * FOE读任务
 */
void foeApp::taskRead(void)
{
    ecx_FOEread_gxf(&ecx_context, 1, (char *)&filename, password, filebuf, 4000000);
}

/**
 * @brief foeApp::setFoeinfo
 * @param size  写的时候表示文件的总大小，读的时候无意义
 * @param name  请求的文件名字
 * @param psword 密码
 */
void foeApp::setFoeinfo(unsigned int size, QString name, uint32 psword)
{
    filesize = size;
    filename = name;
    password = psword;
